...
mean.ebpp$ms_g <- paste(mean.ebpp$ms, "-", mean.ebpp$garden)

mean.ci.bias=c()

# switch between ebpp and ebpps
for (i in 1:length(mean.ebpp$ms_g)) {
  boot.site=bootstrapped.means$ebpp[bootstrapped.means$ms_g==mean.ebpp$ms_g[i]]
  obs.lambda=mean.ebpp$mean.ebpp[mean.ebpp$ms_g==mean.ebpp$ms_g[i]]
  z=qnorm(length(boot.site[boot.site<obs.lambda])/length(boot.site))
  mean.ci.bias$ms_g=mean.ebpp$ms_g
...
ebpp.ci=merge(mean.ebpp,mean.ci.bias,by="ms_g")
...
gg.ebpp.boot <- ggplot(data=(ebpp.ci), aes(x=garden, y=mean.ebpp, colour=ms, group=ms))+
  geom_point(aes(fill=ms), colour="black", pch=21, size=3, position=position_dodge(width=0.2))+
  geom_errorbar(data=ebpp.ci, aes(y=mean.ebpp, ymin=lower.ci, ymax=upper.ci), colour="black", position=position_dodge(width=0.2), width=0.2, alpha=0.5)+
  theme_bw()+
